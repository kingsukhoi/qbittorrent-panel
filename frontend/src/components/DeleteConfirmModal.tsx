	...
	return (
		<Dialog open={isOpen} onClose={onClose} className="relative z-50">
			...
			<div className="fixed inset-0 flex items-center justify-center">
				<DialogPanel className="bg-[var(--qbt-bg-secondary)] border border-[var(--qbt-border)] rounded-lg w-full max-w-sm shadow-xl p-6 space-y-4">
					<DialogTitle className="text-lg font-semibold text-[var(--qbt-text-primary)]">
						Delete {torrents.length} torrent{torrents.length > 1 ? "s" : ""}?
					</DialogTitle>
					<div className="max-h-40 overflow-y-auto rounded border border-[var(--qbt-border)] bg-[var(--qbt-bg-primary)]">
						{torrents.map((t) => (
							<div
								key={t.InfoHashV1}
